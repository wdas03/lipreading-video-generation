...
class Diffusion(nn.Module):
        ...
        with torch.no_grad():
            ...
            for i in trange(n_frames, desc=f'Sampling {mode}'):
                ...
                sample_frame = self.sample_loop(xT[:, i].to(x_cond.device), x_cond, 
                                                motion_frames=motion_frames, audio_emb=None, 
    ...
    def sample_loop(self, xT, x_cond, motion_frames=None, audio_emb=None, i_batch=None, n_batches=None):
        xt = xT
        # Dynamically reduce the noise level as we approach the final frame
        noise_scale = torch.linspace(1.0, 0.1, len(self.timesteps), device=self.device)
        for i, t in enumerate(reversed(self.timesteps)):
            timesteps = torch.full((xT.shape[0],), t, device=xT.device)
            nn_out = self.nn_backbone(xt, timesteps, x_cond)
            mean, logvar = self.get_p_params(xt, timesteps, nn_out)
            noise = torch.randn_like(xt) * noise_scale[i] if t > 0 else torch.zeros_like(xt)
            xt = mean + noise * torch.exp(0.5 * logvar)
        return xt
    ...
    def get_p_params(self, xt, timesteps, nn_out):
        if self.in_channels == self.out_channels:
            eps_pred = nn_out
            # Ensure the beta is on the same device as timesteps before indexing
            p_logvar = self.broadcast(torch.log(self.beta.to(timesteps.device)[timesteps]))
        else:
            eps_pred, nu = nn_out.chunk(2, 1)
            nu = (nu + 1) / 2
            # Also ensure the log_beta_tilde_clipped is on the correct device
            p_logvar = nu * self.broadcast(torch.log(self.beta.to(timesteps.device)[timesteps])) + (1 - nu) * self.broadcast(self.log_beta_tilde_clipped.to(timesteps.device)[timesteps])
    ...
    def get_q_params(self, xt, timesteps, eps_pred=None, x0=None):
        if x0 is None:
            # Ensure coef1_x0 and coef2_x0 are on the same device as timesteps
            coef1_x0 = self.broadcast(self.coef1_x0.to(timesteps.device)[timesteps])
            coef2_x0 = self.broadcast(self.coef2_x0.to(timesteps.device)[timesteps])
            x0 = coef1_x0 * xt - coef2_x0 * eps_pred
            x0 = x0.clamp(-1, 1)

        # q(x_{t-1} | x_t, x_0)
        coef1_q = self.broadcast(self.coef1_q.to(timesteps.device)[timesteps])
        coef2_q = self.broadcast(self.coef2_q.to(timesteps.device)[timesteps])
        q_mean = coef1_q * x0 + coef2_q * xt

        q_logvar = self.broadcast(self.log_beta_tilde_clipped.to(timesteps.device)[timesteps])
...
#     def forward_diffusion(self, x0, timesteps):
#         timesteps = timesteps.to(self.alpha_bar.device)
...
#                 # if stabilize and i > 0 and i % segment_len == 0:
#                 #     motion_frames = torch.cat([self.motion_transforms(x_cond), motion_frames[:, self.nn_backbone.motion_channels:, :]], dim=1)
#                 sample_frame = self.sample_loop(xT[:, i], x_cond, motion_frames=motion_frames, audio_emb=None)
#                 samples.append(sample_frame.unsqueeze(1))
#                 # motion_frames = torch.cat([motion_frames[:, self.nn_backbone.motion_channels:, :], self.motion_transforms(sample_frame)], dim=1)
#                 # audio_ids = audio_ids[1:] + [min(i + self.nn_backbone.n_audio_motion_embs + 1, n_frames - 1)]
#             return torch.cat(samples, dim=1)
...
#             return torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # @property
    # def device(self):
    #     # Attempt to determine the primary device in a DataParallel setup
    #     if hasattr(self, 'module'):
    #         # Access the original module if it's wrapped by DataParallel
    #         params = list(self.module.nn_backbone.parameters())
    #     else:
    #         # Regular approach if not wrapped by DataParallel
    #         params = list(self.nn_backbone.parameters())
        
    #     if params:
    #         # Return the device of the first parameter
    #         return params[0].device
    #     else:
    #         # Default to the first available CUDA device or CPU if none are available
